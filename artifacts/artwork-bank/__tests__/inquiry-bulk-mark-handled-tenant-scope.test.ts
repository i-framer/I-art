/**
 * Regression tests: bulkSetInquiriesStatus is tenant-scoped.
 *
 ...
 * covers status validation, empty-selection, and >200-cap paths.
 */
import { describe, it, expect, beforeEach, vi } from "vitest";

// Billing is validated separately; tenant-scope tests run with the guard stubbed.
vi.mock("@/lib/billing", () => ({
...
describe("bulkSetInquiriesStatus tenant scoping", () => {
  it("scopes a mixed own/foreign ID selection to the session tenant", async () => {
    // "foreign-1"/"foreign-2" belong to another gallery; the tenantId filter
    // in the WHERE clause is what prevents them from being updated.
    const ids = ["own-1", "foreign-1", "own-2", "foreign-2"];
    await bulkSetInquiriesStatus(ids, "HANDLED");
  ...
  it("scopes marking back to NEW by tenant", async () => {
    await bulkSetInquiriesStatus(["own-1", "foreign-1"], "NEW");

    expect(state.updates).toHaveLength(1);
    expect(state.updates[0].vals).toEqual({ status: "NEW" });
    const expectedWhere = and(
  ...
  it("uses tenant-B's tenantId when tenant-B calls the action — cannot reach tenant-A rows", async () => {
    getSession.mockResolvedValue({
      userId: "user-2",
      tenantId: "tenant-B",
      email: "b@example.com",
    });
    await bulkSetInquiriesStatus(["tenant-A-inq-1"], "HANDLED");
    ...
    // WHERE includes tenant-B so tenant-A's row cannot match.
    expect(j(state.updates[0].where)).toEqual(j(expectedWhere));
  });

  it("dedupes and drops empty/non-string IDs before updating", async () => {
    await bulkSetInquiriesStatus(
  ...
  it("rejects an invalid status value without touching the database", async () => {
    await expect(
      bulkSetInquiriesStatus(["own-1"], "DELETED" as any),
  ...
  it("redirects to /login when unauthenticated, touching nothing", async () => {
    ...
    await expect(
      bulkSetInquiriesStatus(["own-1"], "HANDLED"),
