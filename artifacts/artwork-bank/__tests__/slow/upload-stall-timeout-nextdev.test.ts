  ...
  devServer = spawn(
    ...
    ["--filter", "@workspace/artwork-bank", "dev"],
    {
      ...
      env: {
        ...
        // Allow the stall-guard meta-test to inject a short timeout via the
        // UPLOAD_READ_TIMEOUT_MS environment variable.  The constant above is
        // used for assertions; child servers read the env var so that
        // `UPLOAD_READ_TIMEOUT_MS=1 pnpm test:slow` still makes the helper
        // return 408 in ~1 ms, causing `expect(elapsed).toBeGreaterThanOrEqual(1500)`
        // to fail and prove the guard is still effective.
        UPLOAD_READ_TIMEOUT_MS:
          process.env.UPLOAD_READ_TIMEOUT_MS ?? String(UPLOAD_READ_TIMEOUT_MS),
...
async function startHelperServer(
  ...
  helperServer = spawn("pnpm", ["exec", "tsx", helperScript], {
    ...
    env: {
      ...
      // Read from env so the stall-guard meta-test can inject a short timeout
      // (UPLOAD_READ_TIMEOUT_MS=1) and confirm the guard still fires.
      UPLOAD_READ_TIMEOUT_MS:
        process.env.UPLOAD_READ_TIMEOUT_MS ?? String(UPLOAD_READ_TIMEOUT_MS),
  ...
  () => {
    ...
    it(
      ...
      async () => {
        ...
        // statusCode is always assigned in the try block; if fetch throws the
