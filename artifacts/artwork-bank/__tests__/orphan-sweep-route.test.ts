/**
 * Tests for the orphan-sweep API route's authentication guard.
 *
 * Verifies that:
...
// ── Mock the sweep implementation so no DB or storage is touched ───────────────

const sweepOrphanedImageFiles = vi.hoisted(() =>
  vi.fn().mockResolvedValue({ orphaned: 0, deleted: 0, errors: 0, failedPaths: [] })
);
...
describe("orphan-sweep route — authentication guard", () => {
  ...
  describe("production with a secret configured — valid token", () => {
    it("GET with correct Bearer token runs the sweep and returns 200", async () => {
      setEnv({
        NODE_ENV: "production",
        ORPHAN_SWEEP_SECRET: "super-secret-abc",
        CRON_SECRET: undefined,
      });
    ...
    it("accepts CRON_SECRET as an alternative to ORPHAN_SWEEP_SECRET", async () => {
      setEnv({
        NODE_ENV: "production",
        ORPHAN_SWEEP_SECRET: undefined,
