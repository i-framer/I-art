...
// successful send
;
 individual tests can override it via mockRejectedValueOnce
  ...
  return 
{
    ...
    ...actual,
    sendArtworkInquiry: sendArtworkInquirySpy,
...
import 
{
 MAX_EMAIL_ATTEMPTS, sweepUnsentInquiryEmails 
}
...
{
 id: userId, email: `u-${userId}@test.com`, passwordHash: "x" 
}
 as any)
;
    ...
    const 
{
 ...
 = await createTenant("Original Name")
;


    // z.string().optional() rejects null
;
 pass "" for optional fields not under test.
  ...
  it("completes and redirects without logging errors when contactEmail is set but no inquiries need requeuing", async () => 
{
    ...
    // Arrange: tenant starts with a contactEmail
;
 there are no exhausted
    ...
    const 
{
 ...
 = await createTenant("Gallery With No Pending Inquiries")
;
    ...
    const 
{
 ...
 = await createTenant("Gallery With Contact Email")
;
    ...
    const 
{
 ...
 = await createTenant("Gallery Without Email")
;
    ...
    await db.update(tenantsTable).set(
{
 ...
 as any).where(eq(tenantsTable.id, tenantId))
;
    ...
    const 
{
 ...
 = await createTenant("Gallery With Restored Email")
;
...
;
    ...
    // Step 2: clear the contactEmail (stores null)
;
 requeue must NOT fire here.
    const requeueSpy = vi.spyOn(emailSweepModule, "requeueNoContactEmailInquiries")
;
    ...
    const 
{
 ...
 = await createTenant("Gallery With Mixed Errors")
;
    ...
    await db.update(tenantsTable).set(
{
 ...
 as any).where(eq(tenantsTable.id, tenantAId))
;
...
;
    ...
    const afterA = await db.query.inquiriesTable.findFirst(
{
...
;
    ...
    const afterB = await db.query.inquiriesTable.findFirst(
{
    ...
    const 
{
 ...
 = await createTenant("Gallery With Multiple Exhausted Inquiries")
;
      ...
      db.query.inquiriesTable.findFirst(
{
 where: eq(inquiriesTable.id, inquiryId2) 
      ...
      db.query.inquiriesTable.findFirst(
{
 where: eq(inquiriesTable.id, inquiryId3) 
}
),
    ])
;
...
;
  ...
  it("sweep delivers a notification after a gallery changes its contact email (end-to-end)", async () => 
{

    // This test closes the gap left by the requeue tests: those confirm the
    // counter reset (emailAttempts = 0, emailLastAttemptAt = null) but do not
    // verify that sweepUnsentInquiryEmails subsequently picks up the row and
    // processes it.  A regression that resets the counters but somehow excludes
    // the row from the sweep's WHERE clause would pass all prior tests
    // undetected.
    ...
    // Confirm the inquiry is excluded from the sweep before the fix —
    // emailAttempts >= MAX_EMAIL_ATTEMPTS so it won't appear in the WHERE clause.
    const frozen = await db.query.inquiriesTable.findFirst(
{
...
;
    ...
    expect(frozen?.emailAttempts).toBe(MAX_EMAIL_ATTEMPTS)
;


    // Act step 1: gallery owner saves a new contact email.  This calls
    // requeueNoContactEmailInquiries internally, resetting the inquiry's
...
;
    ...
    expect(requeued?.emailAttempts).toBe(0)
;

    expect(requeued?.emailLastAttemptAt).toBeNull()
;


    // Act step 2: run the real email sweep restricted to this tenant so it
    // doesn't disturb rows owned by other test runs.
    ...
    // true triggers the success path (emailSentAt set, emailError cleared).
    ...
    // cleared and emailSentAt is NOT set by the inquiry sweep (it uses
    // emailError: null, not emailSentAt).  Confirm the error was cleared.
    ...
    const 
{
 ...
 = await createTenant("Gallery Direct Requeue Test")
;
