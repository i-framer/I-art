...
describeIntegration(
  ...
  () => {
    ...
    it("replaying a mismatch event does not create a second alert row (idempotency)", async () => {
      const tenantA = await createTenant();
      const tenantB = await createTenant();
      const artworkId = await createArtwork(tenantA); // artwork belongs to tenantA
      const sessionId = `cs_test_${uid()}`;
      ...
      const res1 = await POST(makeRequest(mismatchEvent));
      expect(res1.status).toBe(200);

      // Second delivery of the identical event (Stripe retry / replay).
      const res2 = await POST(makeRequest(mismatchEvent));
    ...
    it("replaying a mismatch event does not overwrite the original slackPostFailed timestamp", async () => {
