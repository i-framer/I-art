...
// The vi.mock() factory above mirrors that shape:
//   { default: { createTransport: createTransportMock } }
//
...
//
// This canary asserts that createTransportMock is actually reached by email.ts,
// so any export-shape mismatch surfaces immediately as a test failure rather
// than an incorrect green build.
describe("nodemailer mock-intercept canary", () => {
  it("createTransportMock is invoked by email.ts — failure here means the vi.mock() wiring is broken", async () => {
    process.env.SMTP_HOST = "smtp.example.com";
