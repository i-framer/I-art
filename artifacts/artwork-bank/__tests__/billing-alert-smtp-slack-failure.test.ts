...
// sendBillingAlertNotification uses the SMTP path, so any export-shape mismatch
// surfaces immediately as a test failure rather than an incorrect green build.
describe("nodemailer mock-intercept canary", () => {
  it("createTransportMock is invoked by sendBillingAlertNotification — failure here means the vi.mock() wiring is broken", async () => {
    process.env.SMTP_HOST = "smtp.example.com";
    process.env.PLATFORM_ADMIN_EMAIL = "admin@example.com";
